    ...
    <style>
        * {
        ...
        .marketplace-title h1 {
            ...
            padding: 14px 28px;
            border-radius: 20px;
            background: linear-gradient(135deg, rgba(0, 59, 149, 0.84) 0%, rgba(74, 144, 226, 0.76) 100%);
            border: 1px solid rgba(255, 255, 255, 0.24);
            box-shadow: 0 14px 32px rgba(0, 33, 84, 0.22);
...
<body>
    ...
    <div id="purchaseModal" class="modal">
        <div class="modal-content">
            ...
            <form id="purchaseForm" class="purchase-form">
                ...
                <button type="submit" class="confirm-btn">BELI</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function attachBankHandlers() {
            document.querySelectorAll('#bankModal .bank-btn').forEach((button) => {
                button.addEventListener('click', async function(event) {
                    event.preventDefault();
                    const bankUrl = this.href;
                    const bankName = this.textContent.replace(/\s+/g, ' ').trim();
