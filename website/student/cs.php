...
<head>
    ...
    <style>
        ...
        :root {
            /* Tukar fail gambar di sini */
            --cs-bg-image: url('ads37/background cs.png'); 
        }
        ...
        .header h1 {
            ...
            padding: 14px 28px;
            border-radius: 20px;
            background: linear-gradient(135deg, rgba(0, 59, 149, 0.84) 0%, rgba(0, 86, 179, 0.70) 100%);
            border: 1px solid rgba(255, 255, 255, 0.22);
            box-shadow: 0 14px 32px rgba(0, 33, 84, 0.24);
...
<body>
    ...
    <script>
        ...
        function openModal(type, price, stokSize) {
            currentProduct = { type, price };
            document.getElementById('modalPrice').textContent = price;
            sizeModal.style.display = "block";
            resetSelections();
            // Render butang size ikut stok
            ...
            sizes.forEach(sz => {
                ...
                btn.textContent = sz;
                if (stokSize && stokSize[sz] == 0) {
                    btn.disabled = true;
                    btn.style.opacity = 0.5;
                    btn.style.cursor = 'not-allowed';
                } else {
                    btn.onclick = function() { selectSize(btn, sz); };
                }
        ...
        document.getElementById('purchaseForm').onsubmit = async function(e) {
            ...
            const result = await createPaymentRecord({
                ...
                product_size: document.getElementById('orderSize').value,
