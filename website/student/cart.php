...
<body>
    ...
    <div id="checkoutModal" class="modal">
        <div class="modal-content">
            ...
            <h3>Maklumat Checkout</h3>
            <form id="checkoutForm" class="checkout-form">
                <div class="form-group">
                    <label for="checkoutCustomerClass">Kelas</label>
                    <input type="text" id="checkoutCustomerClass" required>
                </div>
                <div class="form-group">
                    <label for="checkoutCustomerPhone">No. Telefon</label>
                    <input type="tel" id="checkoutCustomerPhone" required>
                </div>
                <div class="checkout-note">
                    Semua item dalam troli akan direkodkan ke sistem dahulu. Untuk item pakaian yang dimasukkan melalui troli, saiz akan kekal kosong jika belum dipilih sebelum ini.
                </div>
                <button type="submit" class="checkout-submit">Teruskan Ke Bank</button>
            </form>
        </div>
    </div>

    <div id="cartBankModal" class="modal">
        <div class="modal-content">
            <button class="close-modal" onclick="closeCartBankModal()">&times;</button>
            <h3>Pilih Bank</h3>
            <div class="bank-grid">
                <a href="https://www.maybank2u.com.my" target="_blank" class="bank-btn"><img src="ads27/breyer-logo-maybank2.png" alt="Maybank"><span>Maybank</span></a>
                <a href="https://www.cimbclicks.com.my" target="_blank" class="bank-btn"><img src="ads26/breyer-logo-cimb2.png" alt="CIMB"><span>CIMB</span></a>
                <a href="https://www.pbebank.com" target="_blank" class="bank-btn"><img src="ads29/breyer-logo-publicbank2.png" alt="Public Bank"><span>Public Bank</span></a>
                <a href="https://www.hlb.com.my" target="_blank" class="bank-btn"><img src="ads23/breyer-logo-hongleong2.png" alt="Hong Leong"><span>Hong Leong</span></a>
                <a href="https://www.ambank.com.my" target="_blank" class="bank-btn"><img src="ads21/breyer-logo-ambank.png" alt="AmBank"><span>AmBank</span></a>
                <a href="https://www.muamalat.com.my" target="_blank" class="bank-btn"><img src="ads28/breyer-logo-muamalat2.png" alt="Bank Muamalat"><span>Bank Muamalat</span></a>
                <a href="https://www.affinbank.com.my" target="_blank" class="bank-btn"><img src="ads20/breyer-logo-affin2.png" alt="Affin Bank"><span>Affin Bank</span></a>
                <a href="https://www.agrobank.com.my" target="_blank" class="bank-btn"><img src="ads25/breyer-logo-agrbank2.png" alt="Agrobank"><span>Agrobank</span></a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let currentCartPaymentIds = [];
        let currentCartTransactionIds = [];
        ...
        function openCartBankModal() {
            document.getElementById('cartBankModal').style.display = 'flex';
        }

        function closeCartBankModal() {
            document.getElementById('cartBankModal').style.display = 'none';
        }

        async function createCartPayments(payload) {
        ...
        async function saveCartBank(bankName) {
            if (!currentCartPaymentIds.length) {
                return;
            }
            ...
            const response = await fetch('process_purchase.php', {
                ...
                body: new URLSearchParams({
                    action: 'select_bank',
                    payment_ids: JSON.stringify(currentCartPaymentIds),
                    bank_name: bankName,
        ...
        function attachCartBankHandlers() {
            document.querySelectorAll('#cartBankModal .bank-btn').forEach((button) => {
                button.addEventListener('click', async function(event) {
                    event.preventDefault();
                    const bankUrl = this.href;
                    const bankName = this.querySelector('span').textContent.trim();
                    const paymentWindow = window.open('about:blank', '_blank');

                    try {
                        await saveCartBank(bankName);
                    } catch (error) {
                        console.error(error);
                    }
                    ...
                    const firstReference = currentCartTransactionIds[0] || '-';
                    alert(`Checkout troli berjaya direkodkan. Rujukan pertama: ${firstReference}. Sila selesaikan pembayaran melalui ${bankName}.`);
                    cartItems = [];
                    saveCart();
                    loadCartItems();
        ...
        document.getElementById('checkoutForm').addEventListener('submit', async function(event) {
            ...
            const payload = {
                customer_class: document.getElementById('checkoutCustomerClass').value.trim(),
                customer_phone: document.getElementById('checkoutCustomerPhone').value.trim(),
            ...
            const result = await createCartPayments(payload);
            if (!result.success) {
                alert(result.message || 'Checkout troli tidak berjaya.');
                return;
            }

            currentCartPaymentIds = result.paymentIds || [];
            currentCartTransactionIds = result.transactionIds || [];
            closeCheckoutModal();
            alert(`Checkout troli telah direkodkan. Rujukan pertama: ${currentCartTransactionIds[0] || '-'}. Sila pilih Manual Banking atau QR DuitNow dan muat naik bukti bayaran di Sejarah Pembayaran.`);
            window.location.href = 'payment_history.php';
        });
        ...
        window.onclick = function(event) {
            ...
            if (event.target === document.getElementById('cartBankModal')) {
                closeCartBankModal();
            }
        };
    </script>
